...
public class computer {
			...
			public void print() {
				System.out.println("컴퓨터 사망");
			}
			
			class ram{
				public void print() {
					computer.this.print();  					// 외부클래스 매서드 실행
					System.out.println("메모리 사망");{			//오버라이딩 조건: 상속 이거나, 인터페이스
			...
			static class vga{	//내부 static 클래스
				...
				public void print() {
//					computer.this.print();			외부 인스턴스 클래스는 사용하지 못한다.
...
//					- 외부클래스의 인스턴스 변수, 메서드와 관련된 작업을 할 경우 사용
...
//					- 외부 클래스이 객체를 통한 객체생성이 아니라 독자적으로 생성가능
//					- static변수, 메서드는 메모리에 하나만 올라가지만
//					  static 클래스는 여러개 생성이 가능하다.
//					- 외부 클랫의 인스턴스를 사용하지 않는 내부클래스라면
//					  내부클래스를 static으로 선언한다.(일부 IDE 에서는 경고한다.)
...
//					- 일회용 클래스
//					- 익명클래스는 생성자가 없다.
//					- 익명 클래스는 기존의 클래스 메서드를 재정의 하여 사용하는 기법이다.
