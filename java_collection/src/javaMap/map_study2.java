...
public class map_study2 {
	...
	public static void main(String[] args) {
		...
		TreeMap<Integer,String> tree = new TreeMap<>();		//<key,value>
		...
		System.out.println( tree );		// key 값에 대해서 정렬
		...
		// 지정한 키값보다 작은 키값 들을 반환 하다.	- 지정한 키 포함
		// tailMap() - 지정한 키값보다 큰 키값 들을 반환 한다.	- 지정한 키 포합
		// headMap() - tailMap() 의 두번째 인자를 false 넘겨주면 지정한 키는 포함 하지않는다.
		
		
		System.out.println( tree.subMap(200, 600) ); // 200부터 600전까지
		// subMap(from, to) - from 키값 부터 to 키값 전까지에 해당하는 엔트리들을 반환
		
			Properties prop = new Properties();	 	// map 과 유사, <> 제네릭타입이 없다, key와 value 값이 문자열로 이루어져있다
			prop.put("월요일", "한주가 시작한다......");
			prop.put("금요일", "와 내일 부터... 쉬는날이 아니네");
			prop.put("수요일", "수요일 마다 야외 수업?");
			prop.setProperty("화요일", "아직 2일 지났다");		//정석
			...
			while( em.hasMoreElements()) {
				...
				String filename="C:\\java\\java\\java_collection\\src\\javaMap\\router.properties";
